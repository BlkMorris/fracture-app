...
function LeadStory({ story, article, generatedHeadline }: { story: StoryCluster; article: LatestArticle | null; generatedHeadline: string | null }) {
  ...
  const displayHeadline = compactStoryText(bestHeroHeadline(story, article, generatedHeadline), 150);
  ...
  return (
    <Link href={`/story/${story.id}`} className="pulse-lead">
      <div className="pulse-lead-copy">
        ...
        <p className="pulse-meta"><b>{label}</b><span />{formatPulseTime(story.newestArticleAt)}</p>
...
function StoryTile({ story, article }: { story: StoryCluster; article: LatestArticle | null }) {
  ...
  return (
    <Link href={`/story/${story.id}`} className="pulse-story-card">
      <img src={storyImage(story)} alt="" />
      <div>
        <p><b>{label}</b><span />{article?.source.name || formatPulseTime(timestamp)}</p>
        <h2>{headline}</h2>
      </div>
      <PulseFdiBadge score={storyDivergence(story)} compact />
    </Link>
...
function bestHeroHeadline(story: StoryCluster, article: LatestArticle | null, generatedHeadline: string | null) {
  ...
  if (generatedHeadline && !isGeneratedFractureCopy(generatedHeadline)) return generatedHeadline;
  return story.topic;
}

function isGeneratedFractureCopy(value: string) {
  return /^fracture\s+(is\s+)?(tracking|watching|monitoring|comparing|covering)\b/i.test(value.trim());
}
...
function heroDeckSnippet(value: string | null | undefined) {
  ...
  const sentence = clean.match(/^.{24,150}?[.!?](?=\s|$)/)?.[0];
  if (sentence) return sentence.trim();

  const clipped = clean.slice(0, 126);
  const boundary = Math.max(clipped.lastIndexOf(","), clipped.lastIndexOf(";"), clipped.lastIndexOf(" "));
  const snippet = clipped.slice(0, boundary > 70 ? boundary : 126).trim();
...
${pulseChromeStyles}
.pulse-dashboard{display:grid;grid-template-columns:minmax(0,1fr) 388px;gap:18px;padding:8px 20px 18px}.pulse-main-column{min-width:0}.pulse-lead{height:364px;display:grid;grid-template-columns:47% 53%;background:var(--night);color:white;overflow:hidden}.pulse-lead-copy{height:364px;padding:18px 24px 22px;display:flex;flex-direction:column;justify-content:center;min-width:0;overflow:hidden}.pulse-chip,.pulse-image-live{align-self:flex-start;display:inline-flex;align-items:center;gap:7px;background:var(--orange);color:white;font-size:15px;line-height:1;font-weight:950;padding:9px 10px}.pulse-lead h1{max-width:100%;display:-webkit-box;-webkit-line-clamp:4;-webkit-box-orient:vertical;overflow:hidden;color:white;-webkit-text-fill-color:white;font-family:Inter,"Public Sans",Arial,sans-serif;font-size:clamp(54px,5.35vw,88px);line-height:.88;font-weight:1000;font-variation-settings:"wght" 1000;-webkit-font-smoothing:antialiased;text-rendering:geometricPrecision;letter-spacing:-.066em}.pulse-meta,.pulse-story-card p{display:flex;align-items:center;gap:9px;margin:0;color:rgba(255,255,255,.82);font-size:15px}.pulse-meta b,.pulse-story-card b,.pulse-update b{color:var(--cyan);text-transform:uppercase;font-size:12px;font-weight:950}.pulse-meta span,.pulse-story-card p span{width:3px;height:3px;border-radius:999px;background:currentColor}.pulse-deck{max-width:540px;margin:13px 0 0;color:rgba(255,255,255,.9);font-size:18px;line-height:1.3;display:block}.pulse-deck span{display:inline}.pulse-deck em{display:inline-flex;align-items:center;margin-left:10px;color:var(--orange);font-size:12px;line-height:1;font-style:normal;font-weight:950;text-transform:uppercase;white-space:nowrap}.pulse-deck em::after{content:"";width:22px;height:1px;margin-left:7px;background:currentColor}.pulse-watch{display:flex;align-items:center;gap:12px;margin-top:18px;font-size:15px}.pulse-watch i{width:34px;height:34px;border-radius:999px;display:grid;place-items:center;background:white;color:var(--night);font-style:normal}.pulse-watch em{width:1px;height:24px;background:rgba(255,255,255,.42)}.pulse-watch b{color:var(--orange)}
