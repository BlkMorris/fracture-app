...
export default function HomePage() {
  const { data: homepage, isFetching: homepageFetching, isLoading: homepageLoading } = useHomepage();
  const { data: storiesData, isFetching: storiesFetching, isLoading: storiesLoading } = useStories({ limit: 40 });
  const { data: stats } = useStats();
  const contentReady = !homepageLoading && !storiesLoading && !homepageFetching && !storiesFetching;
  ...
  return (
    <main className="pulse-home" aria-label="Fracture Pulse Editorial homepage">
      ...
      <nav className="pulse-tabs" aria-label="Story filters">
        <div>
          <Link className="is-active" href="/stories">Top Stories</Link>
          <Link href="/stories">Newest</Link>
          <Link href="/stories">Watchlist</Link>
        </div>
        <p>Updated {formatPulseTime(hero?.newestArticleAt ?? homepage?.latest?.[0]?.publishedAt)} <span /></p>
      </nav>

      {!contentReady ? <HomepageSkeleton /> : (
        <section className="pulse-dashboard">
          <div className="pulse-main-column">
            {hero ? <LeadStory story={hero} /> : <EmptyState />}
          ...
          <aside className="pulse-updates" aria-label="Live updates">
            ...
            <Link href="/stories" className="pulse-live-blog">
      ...
      {contentReady ? <footer className="pulse-footer">
        <strong>FRACTURE</strong>
        {topics.slice(0, 4).map((topic) => <Link href={`/stories?topic=${encodeURIComponent(topic.label)}`} key={topic.label}>{topic.label}</Link>)}
        <span><i /> Updated {formatPulseTime(hero?.newestArticleAt)}</span>
        <time>{formatClock(hero?.newestArticleAt)}</time>
      </footer> : null}
...
function LeadStory({ story }: { story: StoryCluster }) {
  return (
    <Link href={`/story/${story.id}`} className="pulse-lead">
      <div className="pulse-lead-copy">
        <span className="pulse-chip">LIVE</span>
        <motion.h1 initial={{ y: 18, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.42 }}>
          {story.topic}
        </motion.h1>
        <p className="pulse-meta"><b>{categoryLabel(story.topicCategory)}</b><span />{formatPulseTime(story.newestArticleAt)}</p>
        <p className="pulse-deck">{storySummary(story)}</p>
      ...
      <div className="pulse-lead-image">
        ...
        <span className="pulse-city">{categoryLabel(story.topicCategory).slice(0, 10)}</span>
...
function StoryTile({ story, article }: { story: StoryCluster; article: LatestArticle | null }) {
  const headline = compactStoryText(article?.title || story.topic, 86);
  ...
  return (
    <Link href={`/story/${story.id}`} className="pulse-story-card">
      <img src={storyImage(story)} alt="" />
      <div>
        <p><b>{categoryLabel(story.topicCategory)}</b><span />{article?.source.name || formatPulseTime(timestamp)}</p>
        <h2>{headline}</h2>
      </div>
...
function buildLiveItems(latest: LatestArticle[], stories: StoryCluster[], hero: StoryCluster | null) {
  ...
  const storyItems = [hero, ...stories].filter(Boolean).map((story) => ({
    label: categoryLabel(story!.topicCategory),
    title: story!.topic,
    time: formatClock(story!.newestArticleAt),
    href: `/story/${story!.id}`,
  }));
  return [...articleItems, ...storyItems].slice(0, 5);
}

function buildTimeline(latest: LatestArticle[], stories: StoryCluster[], hero: StoryCluster | null) {
  return buildLiveItems(latest, stories, hero).slice(0, 6);
}
...
${pulseChromeStyles}
.pulse-tabs{min-height:57px;display:flex;align-items:end;justify-content:space-between;gap:24px;padding:0 24px}.pulse-tabs div{display:flex;align-items:flex-end;gap:clamp(28px,4vw,56px)}.pulse-tabs a{position:relative;padding:0 0 13px;font-size:18px;font-weight:900}.pulse-tabs a.is-active::after{content:"";position:absolute;left:0;right:0;bottom:0;height:2px;background:var(--orange)}.pulse-tabs p{display:inline-flex;align-items:center;gap:7px;margin:0 26.5% 12px 0;font-size:16px;white-space:nowrap}.pulse-tabs p span{width:8px;height:8px;border-radius:999px;background:var(--cyan)}
.pulse-dashboard{display:grid;grid-template-columns:minmax(0,1fr) 388px;gap:18px;padding:8px 20px 18px}.pulse-main-column{min-width:0}.pulse-lead{height:364px;display:grid;grid-template-columns:41.5% 58.5%;background:var(--night);color:white;overflow:hidden}.pulse-lead-copy{height:364px;padding:20px 24px 26px;display:flex;flex-direction:column;justify-content:center;min-width:0;overflow:hidden}.pulse-chip,.pulse-image-live{align-self:flex-start;display:inline-flex;align-items:center;gap:7px;background:var(--orange);color:white;font-size:15px;line-height:1;font-weight:950;padding:9px 10px}.pulse-lead h1{margin:18px 0 12px;max-width:640px;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;font-size:clamp(42px,4.2vw,70px);line-height:.98;font-weight:1000;letter-spacing:-.045em}.pulse-meta,.pulse-story-card p{display:flex;align-items:center;gap:9px;margin:0;color:rgba(255,255,255,.82);font-size:15px}.pulse-meta b,.pulse-story-card b,.pulse-update b{color:var(--cyan);text-transform:uppercase;font-size:12px;font-weight:950}.pulse-meta span,.pulse-story-card p span{width:3px;height:3px;border-radius:999px;background:currentColor}.pulse-deck{max-width:500px;margin:18px 0 0;color:rgba(255,255,255,.92);font-size:20px;line-height:1.35;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.pulse-watch{display:flex;align-items:center;gap:13px;margin-top:25px;font-size:16px}.pulse-watch i{width:36px;height:36px;border-radius:999px;display:grid;place-items:center;background:white;color:var(--night);font-style:normal}.pulse-watch em{width:1px;height:26px;background:rgba(255,255,255,.42)}.pulse-watch b{color:var(--orange)}
...
.pulse-footer{min-height:54px;display:flex;align-items:center;gap:36px;padding:0 23px;background:var(--night);color:white}.pulse-footer strong{margin-right:28px;font-size:20px;font-weight:1000;letter-spacing:-.04em}.pulse-footer a{font-size:15px;font-weight:800}.pulse-footer span{margin-left:auto;display:inline-flex;align-items:center;gap:10px}.pulse-footer time{display:inline-flex;align-items:center;gap:10px}.pulse-footer time::before{content:"";width:4px;height:4px;border-radius:999px;background:currentColor}
.pulse-skeleton-lead,.pulse-card-skeleton,.pulse-update-skeleton,.pulse-live-blog-skeleton,.pulse-time-skeleton,.pulse-topic-skeleton,.pulse-global-skeleton,.pulse-skel-line,.pulse-skel-chip,.pulse-skel-actions{background:linear-gradient(90deg,#fff,var(--warm),#fff);background-size:200% 100%;animation:skeleton 1.35s ease-in-out infinite}.pulse-skeleton-lead{min-height:364px;display:grid;grid-template-columns:41.5% 58.5%;background:var(--night);overflow:hidden}.pulse-skel-copy{padding:24px;display:flex;flex-direction:column;justify-content:center;gap:14px}.pulse-skel-media{background:linear-gradient(90deg,rgba(252,252,248,.1),rgba(217,212,204,.52),rgba(252,252,248,.08));background-size:200% 100%;animation:skeleton 1.35s ease-in-out infinite}.pulse-skel-chip{width:52px;height:29px}.pulse-skel-line{display:block;width:78%;height:18px}.pulse-skel-line.is-xl{height:52px;width:92%}.pulse-skel-line.short{width:74%}.pulse-skel-line.narrow{width:58%}.pulse-skel-actions{width:172px;height:36px;margin-top:8px}.pulse-card-skeleton{min-height:202px;border:1px solid var(--line);background:white;display:grid;grid-template-rows:90px 1fr;padding:0}.pulse-card-skeleton span{display:block;background:linear-gradient(90deg,#f8f8f4,var(--warm),#f8f8f4);background-size:200% 100%;animation:skeleton 1.35s ease-in-out infinite}.pulse-card-skeleton i,.pulse-card-skeleton b{display:block;margin:13px 10px 0;height:10px;background:var(--warm);opacity:.9}.pulse-card-skeleton b{width:76%;height:18px}.pulse-card-skeleton i{width:44%}.pulse-rail-skeleton .pulse-section-head{margin-bottom:18px}.pulse-update-skeleton{height:62px;margin:0 0 14px 20px;border-left:1px solid var(--line)}.pulse-live-blog-skeleton{border-color:var(--line);background:linear-gradient(90deg,#fff,var(--warm),#fff);background-size:200% 100%}.pulse-timeline-skeleton .pulse-timeline-track{grid-template-columns:repeat(5,minmax(120px,1fr))}.pulse-time-skeleton{min-height:72px;border-left:1px solid var(--line);background:linear-gradient(90deg,#fff,var(--warm),#fff);background-size:200% 100%;animation:skeleton 1.35s ease-in-out infinite}.pulse-topic-skeleton{min-height:103px;border:1px solid var(--line);background:linear-gradient(90deg,#fff,var(--warm),#fff);background-size:200% 100%;animation:skeleton 1.35s ease-in-out infinite}.pulse-global-skeleton{height:105px;border:1px solid var(--line);background-image:radial-gradient(circle,rgba(16,17,20,.18) 1.15px,transparent 1.3px),linear-gradient(90deg,#fff,var(--warm),#fff);background-size:7px 7px,200% 100%;animation:skeleton 1.35s ease-in-out infinite}.pulse-empty{min-height:364px;border:1px solid var(--night);background:white;display:grid;place-content:center;justify-items:start;padding:34px}.pulse-empty span{color:var(--orange);font-size:12px;font-weight:950;letter-spacing:.12em}.pulse-empty h1{max-width:640px;margin:10px 0 8px;font-size:clamp(42px,5vw,70px);line-height:.92;font-weight:1000;letter-spacing:-.055em}.pulse-empty p{max-width:480px;margin:0 0 22px;color:var(--muted);font-size:18px}.pulse-empty a{border:1px solid var(--night);background:var(--night);color:white;padding:12px 14px;font-weight:950}@keyframes skeleton{to{background-position:-200% 0}}
@media(max-width:1180px){.pulse-dashboard,.pulse-lower{grid-template-columns:1fr}.pulse-tabs p{margin-right:0}.pulse-topic-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media(max-width:780px){.pulse-tabs{align-items:start;flex-direction:column;padding:14px 16px 0}.pulse-tabs div{width:100%;justify-content:space-between;gap:18px}.pulse-tabs a{font-size:16px}.pulse-dashboard{padding:10px 16px 16px}.pulse-lead,.pulse-skeleton-lead{height:auto;grid-template-columns:1fr}.pulse-lead-copy{height:auto;min-height:352px;order:2}.pulse-lead-image{order:1}.pulse-skel-copy{min-height:300px;order:2}.pulse-skel-media{order:1}.pulse-lead h1{font-size:clamp(42px,12vw,58px);line-height:1}.pulse-lead-image,.pulse-skel-media{height:245px;min-height:245px}.pulse-city{right:auto;left:18px}.pulse-story-grid,.pulse-topic-grid,.pulse-global-body{grid-template-columns:1fr}.pulse-timeline{margin-inline:16px;grid-template-columns:1fr}.pulse-timeline-label{min-height:48px}.pulse-timeline-label::after{display:none}.pulse-timeline-track,.pulse-timeline-skeleton .pulse-timeline-track{grid-template-columns:1fr;padding-left:20px}.pulse-time-node,.pulse-time-skeleton{min-height:66px}.pulse-lower{padding-inline:16px}.pulse-footer{flex-wrap:wrap;gap:15px 22px;padding-block:16px}.pulse-footer span{margin-left:0}}
@media(max-width:520px){.pulse-home{overflow-x:hidden}.pulse-tabs{gap:10px;padding:12px 12px 0}.pulse-tabs div{overflow-x:auto;justify-content:flex-start;padding-bottom:1px;scrollbar-width:none}.pulse-tabs div::-webkit-scrollbar{display:none}.pulse-tabs a{flex:0 0 auto;font-size:15px}.pulse-tabs p{font-size:13px;white-space:normal}.pulse-dashboard{padding:10px 12px 14px;gap:14px}.pulse-lead-copy{min-height:0;padding:12px 18px 18px;justify-content:flex-start}.pulse-skel-copy{min-height:218px}.pulse-chip,.pulse-image-live{font-size:13px;padding:8px 9px}.pulse-lead h1{margin:10px 0 9px;font-size:clamp(36px,11vw,48px);line-height:.98;-webkit-line-clamp:4}.pulse-meta{font-size:13px}.pulse-deck{margin-top:12px;font-size:17px;line-height:1.32;-webkit-line-clamp:3}.pulse-watch{flex-wrap:wrap;gap:10px;margin-top:16px}.pulse-lead-image,.pulse-skel-media{height:230px;min-height:230px}.pulse-city{top:16px;left:16px;max-width:calc(100% - 122px);font-size:16px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pulse-image-live{right:14px;top:14px}.pulse-lead-image .pulse-fdi-badge{left:14px;bottom:14px}.pulse-story-grid{gap:12px}.pulse-story-card{height:218px;grid-template-rows:96px 122px}.pulse-story-card>img{height:96px;min-height:96px}.pulse-story-card div{min-height:122px;padding:12px 10px 14px}.pulse-story-card h2{font-size:17px;-webkit-line-clamp:4}.pulse-story-card>.pulse-fdi-badge{right:7px;top:7px}.pulse-update{grid-template-columns:62px minmax(0,1fr);padding-right:0}.pulse-update>span{display:none}.pulse-update p{font-size:14px}.pulse-live-blog{grid-template-columns:1fr auto auto}.pulse-timeline{margin-inline:12px}.pulse-timeline-track,.pulse-timeline-skeleton .pulse-timeline-track{padding-left:12px}.pulse-lower{gap:14px;padding-inline:12px}.pulse-topics,.pulse-global{padding:13px 10px}.pulse-topic{min-height:94px;grid-template-columns:44px minmax(0,1fr) auto;padding:14px;gap:10px}.pulse-topic svg{width:36px;height:36px}.pulse-global-body{gap:12px}.pulse-city-row{grid-template-columns:minmax(0,1fr) auto 42px;gap:8px}.pulse-city-row span{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pulse-map{height:92px}.pulse-footer{align-items:flex-start;gap:12px 18px;padding:15px 12px}.pulse-footer strong{width:100%;margin-right:0}.pulse-footer span{width:100%}}
`;
