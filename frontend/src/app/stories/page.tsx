...
export default function StoriesPage() {
  ...
  const filteredStories = curatedStories.filter((story) => topic === "All" || story.topicCategory?.toLowerCase() === topic.toLowerCase());
      ...
      <nav className="fx-nav" aria-label="Fracture navigation">
        ...
        <div className="fx-nav-row">
          ...
          <div className="fx-topic-nav" role="tablist" aria-label="Filter by topic">
            {TOPICS.map((item) => <button key={item} type="button" className={topic === item ? "active" : ""} onClick={() => setTopic(item)}>{item}</button>)}
          </div>
