...
export default function StoriesPage() {
  ...
  const [activeFilter, setActiveFilter] = useState(() => {
    ...
    if (params.get("status") === "live") return "Live";
    const topic = categoryLabel(params.get("topic"));
  ...
  const { data, isLoading, isFetching, error } = useStories({ limit: 60, search: debouncedQuery || undefined });
  const loadingStories = isLoading;
  const searchSettling = query.trim() !== debouncedQuery;
  const refreshingStories = searchSettling || (isFetching && !isLoading);
  ...
  const filterOptions = useMemo(() => {
    const categories = allStories.map((story) => storyCategoryLabel(story)).filter(Boolean);
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    replaceStoriesUrl(activeFilter, query);
  }

  function clearSearch() {
    setQuery("");
    setDebouncedQuery("");
    replaceStoriesUrl(activeFilter, "");
  }

  function updateFilter(filter: string) {
    setActiveFilter(filter);
    replaceStoriesUrl(filter, query);
  }

  function replaceStoriesUrl(filter: string, search: string) {
    const params = new URLSearchParams();
    if (search.trim()) params.set("search", search.trim());
    if (filter !== "All" && filter !== "Live") params.set("topic", filter);
    if (filter === "Live") params.set("status", "live");
    const qs = params.toString();
    window.history.replaceState(null, "", qs ? `/stories?${qs}` : "/stories");
  }
  ...
  return (
    <main className="pulse-stories" aria-label="Fracture Pulse Editorial stories search">
      ...
      <section className="pulse-search-hero">
        ...
        <form className={`pulse-search-box ${refreshingStories ? "is-searching" : ""}`} onSubmit={handleSubmit}>
          <Search size={28} />
          <input
      ...
      <nav className="pulse-filterbar" aria-label="Search filters">
        <div>
          {filterOptions.map((filter) => (
            <button className={activeFilter === filter ? "is-active" : ""} type="button" onClick={() => updateFilter(filter)} key={filter}>
              {filter}
            </button>
          ))}
        </div>
      ...
      <section className="pulse-stories-layout">
        <div className={`pulse-results ${refreshingStories ? "is-refreshing" : ""}`}>
          ...
          {error ? (
            ...
            <motion.div className="pulse-result-grid" layout transition={{ duration: 0.28, ease: "easeOut" }}>
              <AnimatePresence initial={false}>
                {visibleStories.map((story, index) => (
                  <motion.div
                    className={`pulse-result-motion ${index === 0 ? "is-lead-shell" : ""}`}
                    key={story.id}
                    layout
                  ...
                  >
                    <StoryResult story={story} index={index} />
                  </motion.div>
                ))}
              </AnimatePresence>
            </motion.div>
            ...
            <StoriesState
              title={debouncedQuery ? `No stories found for "${debouncedQuery}".` : "No curated stories are available yet."}
              body={debouncedQuery ? "Try a broader topic, source, or frame. The live index updates as new clusters arrive." : "Fracture is waiting for enough active clusters to populate the story index."}
                ...
                <div className="pulse-watchlist">
                  {watchlist.length ? watchlist.map((story) => (
                    <Link href={`/story/${story.id}`} key={story.id}>
                      <strong>{storyCategoryLabel(story)}</strong>
...
function SearchPulse({ query }: { query: string }) {
  const cleanQuery = query.trim();

  return (
    <div className="pulse-search-pulse" aria-live="polite">
      <span><i /> Tuning search</span>
      <strong>{cleanQuery ? cleanQuery : "Live index"}</strong>
      <div aria-hidden="true"><b /><b /><b /></div>
    </div>
  );
}
...
function buildTags(story: StoryCluster) {
  const tags = [storyCategoryLabel(story), ...story.topicKeywords].filter(Boolean);
  return Array.from(new Set(tags)).slice(0, 3);
}

function buildLiveQueries(stories: StoryCluster[]) {
  const terms = stories.flatMap((story) => [storyCategoryLabel(story), ...story.topicKeywords, story.topic.split(" ").slice(0, 2).join(" ")]);
...
function buildIndex(stories: StoryCluster[]) {
  ...
  stories.forEach((story) => {
    const label = storyCategoryLabel(story);
...
.pulse-results{position:relative}.pulse-search-box{transition:border-color 180ms ease,box-shadow 180ms ease,transform 180ms ease}.pulse-search-box.is-searching{border-color:var(--cyan);box-shadow:10px 10px 0 var(--cyan)}.pulse-search-box.is-searching svg:first-child{color:var(--cyan)}.pulse-search-pulse{min-height:48px;margin:-2px 0 14px;border:1px solid var(--line);background:linear-gradient(90deg,rgba(20,184,200,.1),rgba(255,255,255,.74),rgba(255,90,31,.08));display:grid;grid-template-columns:auto minmax(0,1fr) 92px;align-items:center;gap:12px;padding:0 12px;overflow:hidden;position:relative}.pulse-search-pulse::after{content:"";position:absolute;top:0;bottom:0;left:-30%;width:28%;background:linear-gradient(90deg,transparent,rgba(255,255,255,.86),transparent);animation:pulseSearchSweep 1.2s ease-in-out infinite}.pulse-search-pulse span{display:inline-flex;align-items:center;gap:8px;color:var(--orange);font-size:11px;font-weight:950;letter-spacing:.1em;text-transform:uppercase}.pulse-search-pulse i{width:9px;height:9px;border-radius:999px;background:currentColor;animation:pulseBlink 1.1s ease-in-out infinite}.pulse-search-pulse strong{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:16px;font-weight:1000;letter-spacing:-.03em}.pulse-search-pulse div{display:flex;justify-content:flex-end;gap:5px}.pulse-search-pulse b{width:18px;height:5px;background:var(--cyan);animation:pulseBars .78s ease-in-out infinite}.pulse-search-pulse b:nth-child(2){background:var(--orange);animation-delay:.12s}.pulse-search-pulse b:nth-child(3){animation-delay:.24s}.pulse-results.is-refreshing .pulse-result-grid{opacity:.72;filter:saturate(.86);transition:opacity 180ms ease,filter 180ms ease}.pulse-result-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.pulse-result-motion{min-width:0}.pulse-result-motion.is-lead-shell{grid-column:1/-1}.pulse-result-motion .pulse-result{height:100%}.pulse-result{position:relative;min-height:210px;border:1px solid var(--line);background:white;display:grid;grid-template-columns:210px minmax(0,1fr);overflow:hidden;transition:border-color 160ms ease,transform 160ms ease}.pulse-result:hover{border-color:var(--night);transform:translateY(-2px)}.pulse-result.is-lead{grid-column:1/-1;min-height:320px;grid-template-columns:45% minmax(0,1fr);background:var(--night);color:white}.pulse-result-image{position:relative;min-height:100%;overflow:hidden;background:var(--night)}.pulse-result-image img{width:100%;height:100%;min-height:210px;object-fit:cover;display:block;filter:saturate(.92) contrast(1.08);transition:transform 260ms ease}.pulse-result:hover img{transform:scale(1.035)}.pulse-result-image span{position:absolute;top:18px;left:18px;background:var(--orange);color:white;padding:9px 10px;font-size:14px;font-weight:950}.pulse-result-copy{min-width:0;padding:20px 72px 18px 20px}.pulse-result.is-lead .pulse-result-copy{padding:34px 92px 28px 30px}.pulse-meta{display:flex;flex-wrap:wrap;align-items:center;gap:9px;margin:0;color:var(--muted);font-size:13px;font-weight:800}.pulse-result.is-lead .pulse-meta{color:rgba(255,255,255,.82)}.pulse-meta b{color:var(--cyan);text-transform:uppercase;font-size:12px;font-weight:950}.pulse-meta span{width:3px;height:3px;border-radius:999px;background:currentColor}.pulse-result h3{margin:10px 0 9px;font-size:clamp(24px,2.4vw,38px);line-height:.98;font-weight:1000;letter-spacing:-.045em}.pulse-result.is-lead h3{font-size:clamp(44px,5vw,72px);line-height:.92}.pulse-result-copy>p:not(.pulse-meta){margin:0;color:var(--muted);font-size:15px;line-height:1.28}.pulse-result.is-lead .pulse-result-copy>p:not(.pulse-meta){max-width:560px;color:rgba(255,255,255,.86);font-size:19px}.pulse-tags{display:flex;flex-wrap:wrap;gap:8px;margin-top:18px}.pulse-tags span{border:1px solid var(--line);padding:5px 8px;background:var(--soft);color:var(--night);font-size:12px;font-weight:900}.pulse-result.is-lead .pulse-tags span{border-color:rgba(255,255,255,.22);background:rgba(255,255,255,.1);color:white}.pulse-score{position:absolute;top:16px;right:16px;color:var(--night)}.pulse-result.is-lead .pulse-score .pulse-fdi-badge{box-shadow:5px 5px 0 var(--orange)}
